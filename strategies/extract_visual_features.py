...
def extract_frame_number_from_path(img_path):
	print img_path
	return int(img_path.split('_')[-1])
	...
	def sort_img_paths(img_paths):
	img_path_dict = dict()
	# set up dictionary mapping frame numbers to paths 
	for path in img_paths:
		name = os.path.splitext(path)[0]
		frame_number = extract_frame_number_from_path(name)
		if frame_number is not None:
			img_path_dict[frame_number] = path
	# frame numbers range from 1 to n_frames, so just iterate through these numbers adding the corresponding path to a list
	ordered_paths = []
	for x in xrange(1, len(img_paths)):
		cur_path = img_path_dict[x]
		ordered_paths.append(cur_path)
...
>>>>>>> fe4c7491d293659699dbf27ac074ff2c56261ab2
